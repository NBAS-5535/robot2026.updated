...
public class RobotAlignCommandWithLimeLight extends Command {
  ...
  private  final PIDControllerConfigurable rotationalPidController = 
                    new PIDControllerConfigurable(0.05, 0., 0., kSetpoint, kSetpointTolerance); // tol=1deg
    ...
    if (LimelightHelpers.getTV("limelight")) {
      ...
      for (RawFiducial fiducial : fiducials) {
        if (fiducial.id == m_tagId) {
          ...
          SmartDashboard.putNumber("RobotAlignCommandWithLimeLight/distToRobot", fiducial.distToRobot);
